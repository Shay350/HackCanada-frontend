    ...
    <div style={{ 
      ...
      >
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '0.125rem' }}>
          ...
          <div style={{ display: 'flex', gap: '0.375rem', marginTop: '0.375rem' }}>
             ...
             {incident.status !== 'online' && (
                <span style={{ fontSize: '0.625rem', backgroundColor: 'rgba(239, 68, 68, 0.2)', color: '#FCA5A5', padding: '0.125rem 0.375rem', borderRadius: '4px', fontWeight: 600 }}>Action Required</span>
             )}
          </div>
        </div>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '0.25rem', fontSize: '0.8125rem', color: 'var(--text-secondary)' }}>
           <span>{incident.id === 'inc-012' ? '100.89.33.114' : '100.100.6.57'}</span>
           <ChevronDown size={14} color="var(--text-muted)" />
        </div>

        {/* Version Column */}
        <div style={{ display: 'flex', flexDirection: 'column', fontSize: '0.8125rem', color: 'var(--text-secondary)' }}>
           <span style={{ display: 'flex', alignItems: 'center', gap: '0.375rem' }}>
              <span style={{ color: incident.status === 'issue' ? 'var(--status-issue)' : 'var(--text-muted)' }}>
                 {incident.status === 'issue' ? '!' : '+'}
              </span>
              1.94.2
           </span>
           <span style={{ color: 'var(--text-muted)' }}>Linux 6.8.0-generic</span>
        </div>

        {/* Last Seen Column */}
        <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', fontSize: '0.8125rem' }}>
           <span className={`status-dot ${incident.status === 'online' ? 'online' : incident.status}`}></span>
           <span style={{ color: incident.status === 'online' ? 'var(--text-primary)' : incident.status === 'issue' ? 'var(--status-issue)' : 'var(--text-primary)' }}>
              {incident.status === 'resolving' ? 'Diagnosing' : incident.status === 'issue' ? 'Offline (Down)' : 'Connected'}
           </span>
        </div>
      ...
      {expanded && (incident.status === 'issue' || incident.status === 'resolving') && (
        <div style={{ padding: '0 1rem 1.5rem 1rem', display: 'flex', flexDirection: 'column', gap: '1rem', borderTop: '1px solid rgba(255,255,255,0.05)', backgroundColor: 'rgba(255,255,255,0.01)' }}>
          ...
          <div style={{ backgroundColor: '#000000', borderRadius: 'var(--radius-md)', padding: '1rem', fontFamily: '"JetBrains Mono", ui-monospace, SFMono-Regular, Menlo, Monaco, Consolas, monospace', fontSize: '0.8125rem', color: '#E5E7EB', border: '1px solid #333' }}>
          ...
          {incident.proposedFix && (
            <div style={{ border: '1px solid var(--borderColor)', borderRadius: 'var(--radius-md)', padding: '1.25rem', backgroundColor: 'var(--bg-surface)', display: 'flex', flexWrap: 'wrap', gap: '1.5rem', justifyContent: 'space-between', alignItems: 'center' }}>
              ...
              <div className="flex gap-2">
                 ...
                 <button className="btn btn-secondary" onClick={(e) => { e.stopPropagation(); setExpanded(false); }}>
                    Dismiss
                 </button>
              </div>
            </div>
